#!/usr/bin/env bash
# GSPO training on SWE-rebench with an AGS coding-agent harness, 4 nodes.
#
# Thin wrapper, same pattern as run_cbc_*.sh and run_ppo_*.sh: the launcher
# logic lives in run_cc_qwen35_35b_a3b_swe_4nodes.sh and is estimator-agnostic
...
#   SWE_AGENT=codebuddy_code bash run_gspo_qwen35_35b_a3b_swe_4nodes.sh
#
# Usage (from a long-lived shell / tmux on the head node, inside the container):
#   bash examples/coding_agent_rl_ags/run_gspo_qwen35_35b_a3b_swe_4nodes.sh
set -euo pipefail
...
export EXP_TAG="${EXP_TAG:-coding_agent_rl_ags_gspo_${SWE_AGENT}_qwen35_35b_a3b_4nodes}"
