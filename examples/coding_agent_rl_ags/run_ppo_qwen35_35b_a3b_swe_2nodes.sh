...
# run_cc_qwen35_35b_a3b_swe_2nodes.sh and is estimator-agnostic --
...
#
# Everything the CC launcher accepts works here too, plus the PPO knobs from
# algo_args.sh (CRITIC_LR, NUM_CRITIC_ONLY_STEPS, CRITIC_SAVE_DIR,
# CRITIC_LOAD_DIR, PPO_GAMMA, PPO_LAMBD, PPO_VALUE_CLIP). Pick the harness with
# SWE_AGENT, e.g.
#   SWE_AGENT=codebuddy_code bash run_ppo_qwen35_35b_a3b_swe_2nodes.sh
#
# Usage (from a long-lived shell / tmux on the head node, inside the container):
#   bash examples/coding_agent_rl_ags/run_ppo_qwen35_35b_a3b_swe_2nodes.sh
set -euo pipefail
...
export EXP_TAG="${EXP_TAG:-coding_agent_rl_ags_ppo_${SWE_AGENT}_qwen35_35b_a3b_2nodes}"
